...
const Chart = () => {
  useEffect(() => {
    ...
    const theme: any = dashboard.getTheme()
    ...
    createSpectrumDataGenerator()
      ...
      .then((data: any[][]) => {
        ...
        const showProjection = (axisX: number, axisY: number) => {
          // Calculate spectrogram 1D projections at axis location for both X and Y planes.
          let projectionY
          try {
            projectionY = data[Math.round(axisX)].map((value: any, i: any) => ({
              x: value,
              y: i,
            }))
          } catch (e) { }
  ...
  return <div id='chart' className='chart' style={{ height: '800px' }}>
  </div>
}
